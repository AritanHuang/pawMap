import{S as c}from"./bootstrap.min-2015cc09.js";import{a,b as n,u as d}from"./axios-744198d0.js";const r=document.querySelector("#pet-collect-list"),f=document.querySelector("#btn-delete-collectall");function p(){let l=[];a.get(`${n}/collects?userId=${d}&_expand=hospital`).then(e=>{l=e.data,l.length>0?t():(c.fire({title:"尚未有收藏紀錄",text:"目前還沒有收藏紀錄",icon:"warning",footer:'<a class="text-primary" href="hospital.html">前往寵物醫院列表收藏</a>'}),r.innerHTML='<p class="fs-1 text-center ">目前尚無收藏項目</p>',f.classList.add("d-none"))}).catch(e=>{console.log(e)});function t(){let e="";l.forEach(function(s,o){e+=` <li class="col-12 col-sm-6 col-lg-4 ">
                            ...
                            </li>`}),r.innerHTML=e}}p();r.addEventListener("click",function(l){const t=l.target.closest(".btn-delete-collect");if(t){if(t){l.preventDefault();const e=t.dataset.id;$(e,t)}}else return});function $(l,t){a.get(`${n}/collects?hospitalId=${l}&userId=${d}`).then(e=>{e.data&&a.delete(`${n}/collects/${e.data[0].id}`).then(s=>{c.fire(`取消收藏${t.dataset.name}`),g(t.dataset.id),p()})}).catch(e=>{console.log(e),c.fire("刪除失敗請稍後再試!")})}function g(l){a.get(`${n}/hospitals/${l}`).then(t=>{const e=t.data.被收藏次數-1;return a.patch(`${n}/hospitals/${l}`,{被收藏次數:e})}).then(t=>{}).catch(t=>{c.fire("發生錯誤請稍後再試")})}f.addEventListener("click",function(l){l.preventDefault(),a.get(`${n}/collects?userId=${d}`).then(t=>{const e=t.data,s=e.map(o=>a.delete(`${n}/collects/${o.id}`));return Promise.all(s).then(o=>{const h=e.map(i=>a.get(`${n}/hospitals/${i.hospitalId}`).then(m=>{const u=m.data.被收藏次數-1;return a.patch(`${n}/hospitals/${i.hospitalId}`,{被收藏次數:u})}));return Promise.all(h).then(i=>{p()})})}).catch(t=>{console.log(t)})});
