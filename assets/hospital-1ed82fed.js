import{S as p}from"./bootstrap.min-2015cc09.js";import{a as g,b as h}from"./axios-744198d0.js";import{a as q,s as b}from"./utils-6feeab9a.js";const v=document.querySelector("#hospital-list"),k=document.querySelector("#pagination");let f=[],d=[],y=0,m=12,n=1,u=[];function L(){g.get(`${h}/hospitals`).then(function(e){f=e.data,y=f.length,x(1),w()}).catch(function(e){console.log(e)})}L();function $(){const e=document.querySelector("#hospital-search");let l="",t=0;d.forEach(function(a){let s="";if(a.機構地址){let o=a.機構地址;s=b(o)}l+=` <li class="col-12 col-sm-6 col-lg-4 ">
        ...
        </li>`,t++}),v.innerHTML=l,t===0&&p.fire("很抱歉，本區域尚無資料"),e.textContent=`本次搜尋共${t}筆`}function P(e,l){let t="";const a=Math.ceil(e/l),s=5;let o=1,r=a;if(a>s){const i=Math.floor(s/2);o=n-i,r=n+i,o<1?(o=1,r=s):r>a&&(r=a,o=a-s+1)}n===1?t+=`<li class="page-item">
        ...
        <a class="page-link disabled" href="#" aria-label="Next">
            <span aria-hidden="true">&raquo;</span>
                ...
                </li>`,k.innerHTML=t,document.querySelectorAll(".page-link").forEach(i=>{i.addEventListener("click",function(c){c.preventDefault();const S=c.target.getAttribute("data-id");c.currentTarget.getAttribute("id")==="next-link"?n++:c.currentTarget.getAttribute("id")==="previous-link"?n--:n=parseInt(S),x(n)})})}function x(e){g.get(`${h}/hospitals?_page=${e}&_limit=${m}`).then(l=>{d=l.data,$(),P(y,m)}).catch(l=>{console.log(l)})}function w(){g.get(`${h}/areas`).then(e=>{u=e.data,D()}).catch(e=>{console.log(e)})}function D(){const e=document.querySelector("#district");let l='<option value="" disabled selected>請選擇行政區</option>';u.forEach(t=>{l+=` <option value="${t.englishName}">${t.name}</option>`}),e.innerHTML=l,e.addEventListener("change",t=>{d=[],u.forEach(a=>{a.englishName===t.target.value&&f.forEach(s=>{b(s.機構地址)===a.name&&d.push(s)})}),$(),k.classList.add("d-none")})}const E=document.querySelector("#top-link");E.addEventListener("click",e=>{e.preventDefault(),q()});v.addEventListener("click",e=>{e.target.classList.contains("btn-collect")&&(e.preventDefault(),localStorage.getItem("token")?p.fire({icon:"success",title:"成功收藏",showConfirmButton:!1,timer:1500}):p.fire({icon:"error",title:"Oops...",text:"請先登入",footer:'<a href="login.html">登入</a>'}))});
